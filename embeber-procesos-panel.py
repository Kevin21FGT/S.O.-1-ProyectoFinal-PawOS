...
# -*- coding: utf-8 -*-
"""
Convierte "Administracion de Procesos" de ventana emergente a panel
embebido. No toca cargar_procesos(), on_refrescar_procesos_clicked,
on_crear_proceso_clicked, on_terminar_proceso_clicked, ni la
restriccion de "solo Admin" en on_procesos_clicked.
"""
import shutil
import sys
...
ANCLA_CLICKED = r'''static void on_procesos_clicked(GtkButton *boton, gpointer datos) {
    (void)boton;
...
NUEVO_CLICKED = r'''static void on_procesos_clicked(GtkButton *boton, gpointer datos) {
    (void)boton;
    DatosBotonModulo *d = (DatosBotonModulo *)datos;
    if (d->rol != ROL_ADMIN) {
        mostrar_mensaje(GTK_WINDOW(d->ventana_principal),
...
ANCLA_ABRIR = r'''static void abrir_pantalla_procesos(GtkWidget *padre) {
...
NUEVO_ABRIR = r'''static void abrir_pantalla_procesos(GtkWidget *area_dinamica, GtkWidget *ventana_principal) {
    ContextoProcesos *ctx = g_malloc0(sizeof(ContextoProcesos));
...
def main():
    ...
    print("Administracion de Procesos ahora se abre embebida en el panel principal")
