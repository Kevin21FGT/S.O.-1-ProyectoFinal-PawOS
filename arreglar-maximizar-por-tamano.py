...
# -*- coding: utf-8 -*-
"""
Reemplaza el mecanismo del boton de maximizar/restaurar por uno mas
simple y a prueba de fallos: en vez de confiar en el estado que reporta
GTK/el gestor de ventanas (gtk_window_is_maximized(), o una señal
window-state-event que ademas tenia un error real: la funcion conectada
debia devolver gboolean y devolvia void -- un descuido del parche
anterior), ahora se compara el tamaño ACTUAL de la ventana contra el
tamaño del monitor donde esta en el momento del clic.

Esto es a prueba de fallos porque no depende de que el gestor de
ventanas haya confirmado o no un cambio de estado -- simplemente mide
los pixeles reales de la ventana ahora mismo, sin importar si llego a
ese tamaño con el boton, arrastrando una esquina, o de cualquier otra
forma.

Quita por completo la señal window-state-event y la funcion
on_estado_ventana_cambio del parche anterior (ya no hacen falta).
"""
import shutil
import sys
...
ANCLA = r'''static void on_estado_ventana_cambio(GtkWidget *ventana, GdkEventWindowState *evento, gpointer datos) {
...
NUEVO = r'''/* En vez de confiar en lo que reporta GTK/el gestor de ventanas sobre
...
def main():
    ...
    print("  - Boton de maximizar/restaurar ahora compara tamaño real vs tamaño de pantalla")
    print("  - Se quito la señal window-state-event (tenia un error de tipo de retorno)")
