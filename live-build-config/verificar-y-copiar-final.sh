#!/bin/bash
# verificar-y-copiar-final.sh - Verifica dentro del ISO recien construido
# que esten: el binario con el boton, el script actualizador, el .desktop,
# y git. Si todo esta bien, copia el ISO a la carpeta compartida con
# nombre nuevo (timestamp) para evitar cualquier confusion de VirtualBox.
set -e
...
echo ""
echo "=== 2) script actualizador presente, sin url/ramas visibles ==="
grep -c "Novedades de esta actualizacion" /tmp/squashfs-root/usr/local/bin/pawos-actualizar-gui || echo "0 (FALTA)"
grep -c "github.com" /tmp/squashfs-root/usr/local/bin/pawos-actualizar-gui || echo "0 (bien, no deberia aparecer)"

echo ""
echo "=== 3) lanzador .desktop presente ==="
