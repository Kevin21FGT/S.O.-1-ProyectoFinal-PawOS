#!/bin/bash
# agregar-compilador.sh - Agrega las herramientas de compilacion
# (build-essential, pkg-config, libgtk-3-dev) que el actualizador
# necesita para recompilar pawos-refugio y pawos-refugio-gui.
set -e
...
echo ""
echo "=== instalando herramientas de compilacion en el chroot ==="
sudo cp /etc/resolv.conf chroot/etc/resolv.conf
sudo mount --bind /dev chroot/dev
sudo mount --bind /proc chroot/proc
sudo mount --bind /sys chroot/sys
sudo chroot chroot apt-get update
sudo chroot chroot apt-get install -y build-essential pkg-config libgtk-3-dev
...
printf "build-essential\npkg-config\nlibgtk-3-dev\n" | sudo tee package-lists/pawos-build-tools.list.chroot
sudo mkdir -p config/package-lists
printf "build-essential\npkg-config\nlibgtk-3-dev\n" | sudo tee config/package-lists/pawos-build-tools.list.chroot
...
git commit -m "Agrega herramientas de compilacion (requeridas por el actualizador)"
